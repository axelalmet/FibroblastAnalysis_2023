...
##################################################################################################################################################################
##################                            Analyse CCC between fibroblast and immune cells                             ########################################
##################################################################################################################################################################

### Unwounded P21
uwp21.meta.data <- integrated.meta.data[(integrated.meta.data$sample == "UW P21"), ]
uwp21.meta.data$leiden_sub <- droplevels(uwp21.meta.data$leiden_sub)

# Get the relevant immune and fibroblast clusters of interest
uwp21_immune_clusters_of_interest <- c("DC-1", "DC-3", "MAC-1", "MAC-2", "MAC-5", "BASO", "MAST", "NEU", "NK", "TCELL", "pDC")
uwp21_fib_clusters_of_interest <- c("DP",  "FIB-VIII")

uwp21.meta.data.subset <- uwp21.meta.data[(uwp21.meta.data$leiden_sub %in% uwp21_immune_clusters_of_interest)|
                                            ((uwp21.meta.data$leiden_sub %in% uwp21_fib_clusters_of_interest)),] 
uwp21.meta.data.subset$leiden_sub <- droplevels(uwp21.meta.data.subset$leiden_sub)

uwp21.use <- rownames(uwp21.meta.data.subset)
uwp21.data.input <- integrated.data.input[, uwp21.use]

uwp21.identity <- data.frame(group = uwp21.meta.data.subset$leiden_sub, row.names = row.names(uwp21.meta.data.subset))

# Create the cellchat object
uwp21.cc <-createCellChat(object = uwp21.data.input, do.sparse = T, meta = uwp21.identity, group.by = "group")
levels(uwp21.cc@idents) # show factor levels of the cell labels

uwp21GroupSize <- as.numeric(table(uwp21.cc@idents)) # Get the number of cells in each group
uwp21MinCells <- floor(min_percentage / 100.0 * sum(uwp21GroupSize))

uwp21.cc@DB <- CellChatDB.use # Set the database for the unwounded data
...
uwp21.cc <- subsetData(uwp21.cc) # We subset the expression data of signalling genes to save on computational cost
uwp21.cc <- identifyOverExpressedGenes(uwp21.cc) # Identify over-expressed genes (I wonder how much the pre-processing in Seurat has an effect on this)
uwp21.cc <- identifyOverExpressedInteractions(uwp21.cc) # Identify the over-expressed ligand-receptor interactions, which are determined by an over-expressed ligand OR receptor
uwp21.cc <- projectData(uwp21.cc, PPI.mouse) # Other option includes PPI.human We're told that we may have to comment these out.

# We now infer the cell-cell communication network by calculating the communication probabilities
uwp21.cc <- computeCommunProb(uwp21.cc, raw.use = FALSE, population.size = FALSE)
uwp21.cc <- filterCommunication(uwp21.cc, min.cells = uwp21MinCells) # Filter out the clusters with small numbers
uwp21.cc <- computeCommunProbPathway(uwp21.cc) # Calculate the probabilities at the signalling level
uwp21.cc <- aggregateNet(uwp21.cc) # Calculates the aggregated cell-cell communication network by counting the links or summing the communication probabilities

uwp21.net <- subsetCommunication(uwp21.cc)

### Unwounded P49
uwp49.meta.data <- integrated.meta.data[(integrated.meta.data$sample == "UW P49"), ]
uwp49.meta.data$leiden_sub <- droplevels(uwp49.meta.data$leiden_sub)

# Get the relevant immuen adn fibroblast clusters of interest
uwp49_immune_clusters_of_interest <- c("DC-1", "DC-2", "DC-3", "MAC-1", "MAC-2", "MAC-3", "BASO", "NK", "TCELL")
uwp49_fib_clusters_of_interest <- c("FIB-III")

uwp49.meta.data.subset <- uwp49.meta.data[(uwp49.meta.data$leiden_sub %in% uwp49_immune_clusters_of_interest)|
                                            ((uwp49.meta.data$leiden_sub %in% uwp49_fib_clusters_of_interest)),] 
uwp49.meta.data.subset$leiden_sub <- droplevels(uwp49.meta.data.subset$leiden_sub)

uwp49.use <- rownames(uwp49.meta.data.subset)
uwp49.data.input <- integrated.data.input[, uwp49.use]

uwp49.identity <- data.frame(group = uwp49.meta.data.subset$leiden_sub, row.names = row.names(uwp49.meta.data.subset))

# Create the cellchat object
uwp49.cc <-createCellChat(object = uwp49.data.input, do.sparse = T, meta = uwp49.identity, group.by = "group")
levels(uwp49.cc@idents) # show factor levels of the cell labels

uwp49GroupSize <- as.numeric(table(uwp49.cc@idents)) # Get the number of cells in each group
uwp49MinCells <- floor(min_percentage / 100.0 * sum(uwp49GroupSize))

uwp49.cc@DB <- CellChatDB.use # Set the database for the unwounded data

# We now identify over-expressed ligands/receptors in a cell group and then project gene expression data onto the protein-protein interaction network
uwp49.cc <- subsetData(uwp49.cc) # We subset the expression data of signalling genes to save on computational cost
uwp49.cc <- identifyOverExpressedGenes(uwp49.cc) # Identify over-expressed genes (I wonder how much the pre-processing in Seurat has an effect on this)
uwp49.cc <- identifyOverExpressedInteractions(uwp49.cc) # Identify the over-expressed ligand-receptor interactions, which are determined by an over-expressed ligand OR receptor
uwp49.cc <- projectData(uwp49.cc, PPI.mouse) # Other option includes PPI.human We're told that we may have to comment these out.

# We now infer the cell-cell communication network by calculating the communication probabilities
uwp49.cc <- computeCommunProb(uwp49.cc, raw.use = FALSE, population.size = FALSE)
uwp49.cc <- filterCommunication(uwp49.cc, min.cells = uwp49MinCells) # Filter out the clusters with small numbers
uwp49.cc <- computeCommunProbPathway(uwp49.cc) # Calculate the probabilities at the signalling level
uwp49.cc <- aggregateNet(uwp49.cc) # Calculates the aggregated cell-cell communication network by counting the links or summing the communication probabilities

uwp49.net <- subsetCommunication(uwp49.cc)
...
swpwd4_fib_clusters_of_interest <- c("FIB-III","FIB-IV")

swpwd4.meta.data.subset <- swpwd4.meta.data[(swpwd4.meta.data$leiden_sub %in% swpwd4_immune_clusters_of_interest)|
                                              ((swpwd4.meta.data$leiden_sub %in% swpwd4_fib_clusters_of_interest)),] 
...
swpwd7_fib_clusters_of_interest <- c("FIB-II", "FIB-III", "FIB-X")

swpwd7.meta.data.subset <- swpwd7.meta.data[(swpwd7.meta.data$leiden_sub %in% swpwd7_immune_clusters_of_interest)|
                                              ((swpwd7.meta.data$leiden_sub %in% swpwd7_fib_clusters_of_interest)),] 
...
lwpwd12.meta.data.subset <- lwpwd12.meta.data[(lwpwd12.meta.data$leiden_sub %in% lwpwd12_immune_clusters_of_interest)|
                                                ((lwpwd12.meta.data$leiden_sub %in% lwpwd12_fib_clusters_of_interest)),] 
...
lwpwd18.meta.data.subset <- lwpwd18.meta.data[(lwpwd18.meta.data$leiden_sub %in% lwpwd18_immune_clusters_of_interest)|
                                                ((lwpwd18.meta.data$leiden_sub %in% lwpwd18_fib_clusters_of_interest)),] 
...
write.csv(uwp21.net, file = "../output/integratedfibroblasts_communications_fibimm_uwp21.csv")
write.csv(uwp49.net, file = "../output/integratedfibroblasts_communications_fibimm_uwp49.csv")
write.csv(swpwd4.net, file = "../output/integratedfibroblasts_communications_fibimm_swpwd4.csv")
write.csv(swpwd7.net, file = "../output/integratedfibroblasts_communications_fibimm_swpwd7.csv")
write.csv(lwpwd12.net, file = "../output/integratedfibroblasts_communications_fibimm_lwpwd12.csv")
write.csv(lwpwd18.net, file = "../output/integratedfibroblasts_communications_fibimm_lwpwd18.csv")
